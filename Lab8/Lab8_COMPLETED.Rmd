...
levels(physq_long_genera$genus) #View all genera

genus_colors <- c("#e393b9",
"#35bc42",
"#8b26aa",
"#5fb932",
"#ae4acd",
...
genusplot <- ggplot(data = physq_long_genera, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
           fill = genus)) + #set fill to the genus rank
  geom_col() + #plot type for bars 
  facet_grid(~semester) + #panel for each semester
  xlab(NULL) + 
  theme(axis.text.x = element_text(angle = 90,
  ...
  # improve plot legend readability 
  scale_fill_manual(name = "Genera", #change name for legend
                    values = genus_colors) #indicate colors from vector of custom colors 

save_plot("~/Desktop/UMich_Bio201_F19/Lab8/figures/genusplot.pdf",
          plot = genusplot)
```
...
levels(physq_long_order$order)

order_color <- c("#965689",
"#5cc451",
"#cc52c1",
"#79ac3c",
"#7863d9",
...
orderplot <- ggplot(data = physq_long_order, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
           fill = order)) + #set fill to the genus rank
  geom_col() + #plot type for bars 
  facet_grid(~semester) + #panel for each semester
  xlab(NULL) + 
  theme(axis.text.x = element_text(angle = 90,
  ...
  scale_fill_manual(name = "Order", #change name for legend
                    values = order_color) #indicate colors from vector of custom colors 

ggsave("~/Desktop/UMich_Bio201_F19/Lab8/figures/orderplot.png",
          plot = orderplot) 
```
...
```{r}
ordplotj <- plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "semester", 
                     shape = "study_week")
...
#The low-p values (0.001 for both semester and study week) mean that the clustering of samples by semester and study week is statistically significant. Based upon the R^2 and low R value (R2 = 0.02871, 0.00774 for semester/studyweek respectively), there is also a weak correlation between the study week/semester and the species data. Therefore, the results can be attributed to variance within the sample. 
```
