...
```{r}
scfa_data <- read_delim(file = "Curated Data/scfa_indv_qc.txt",
...
scfa_indv_qc %>%
  ...
  summarize(mean_butyrate = mean(butyrate_mmol_kg), mean_propionate = mean(propionate_mmol_kg), mean_acetate = mean(acetate_mmol_kg))
```
...
```{r}
butyrate_diff <- scfa_data %>%
  filter(study_week = "week1" | "week3") %>%
  spread(supplement_consumed, but_mean) %>%
  filter(supplement_consumed = "BRMPS") %>%
  mutate(but_diff = "week3" - "week1")
```

Repeat this process for each SCFA. How would you plot your results? 
```{r}
acetate_diff <- scfa_data %>%
  filter(study_week = "week1" | "week3") %>%
  spread(supplement_consumed, ace_mean) %>%
  filter(supplement_consumed = "acetate") %>%
  mutate(ace_diff = "week3" - "week1")
```

```{r}
propionate_diff <- scfa_data %>%
  filter(study_week = "week1" | "week3") %>%
  spread(supplement_consumed, pro_mean) %>%
  filter(supplement_consumed = "propionate") %>%
  mutate(ace_diff = "week3" - "week1")
```
...
butyrate_diff <- scfa_data %>%
  ...
  labs(x = "Individuals",
       y = "Difference in Butyrate (mmol/kg)") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")

butyrate_diff + geom_blank() 

[Repeat for propionate and acetate]
```
