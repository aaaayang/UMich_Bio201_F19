...
```{r}
tax_scfa <- merge(scfa_long, taxa_cat_curated,
                 by = "participant_id")
```

# Intro

Last week we discussed the factors influencing data structure (sample size, variance), how to check assumptions for statistical tests, how to determine if the differences between two means (t-tests, Mann-Whitney-U) or more (ANOVA) are likely real, and what to do if your datset does not meet the assumptions when comparing two means (non-parametric). 
...
```{r}
# hypothesis: Hydrogen breath levels will significantly differ between participants consuming fiber supplements. 

# plot 
h2_data <- breath_wkly %>%
  select(-starts_with("ch4"), -ends_with("median"),
         -starts_with("co2"), -starts_with("n"), -starts_with("comments")) %>%
  filter(quantity_compliant == "yes")
h2_data

plot_supp <- ggplot(h2_data, aes(x = supplement_consumed, y = h2_mean, color = supplement_consumed)) + geom_violin()
plot_supp 
```

* Which assumptions need checked for the statistical test(s)? Check the assumptions in the space below:
```{r}
#Assumptions to be checked include: Sample size, Normality, and Variance. 

#Sample Size of Whole Set:
length(h2_data$participant_id) #n = 415. Demonstrates large, experimentally valid sample size.
group_by(h2_data) %>% summarize(sample_size = n()) #n = 415. Check. Same result. 

#Accessible:
h2_data_acc <- h2_data %>%
  filter(supplement_consumed == "Accessible")
length(h2_data_acc$participant_id) #n = 24. Sample size small. 
shapiro.test(h2_data_acc$h2_mean) #p = 0.0937. Suggests normality as p > 0.05. 
hist(h2_data_acc$h2_mean) 

#Arabino:
h2_data_ara <- h2_data %>%
  filter(supplement_consumed == "Arabino")
length(h2_data_ara$participant_id) #n = 18. Sample size small.  
shapiro.test(h2_data_ara$h2_mean) #p = 0.1695. Normal
hist(h2_data_ara$h2_mean) #Normality questionable. Data not particularly normal. 

#Banana:
h2_data_ban <- h2_data %>%
  filter(supplement_consumed == "Banana")
length(h2_data_ban$participant_id) #n = 31. Sample size mdoerate as n > 25.  
shapiro.test(h2_data_ban$h2_mean) #p = 0.004114
hist(h2_data_ban$h2_mean)

#BRMPS:
h2_data_brmps <- h2_data %>%
  filter(supplement_consumed == "BRMPS")
length(h2_data_brmps$participant_id) #n = 154. Large sample size. 
shapiro.test(h2_data_brmps$h2_mean) #Non-normal sample group. p = 2.823*10^(-12)
hist(h2_data_brmps$h2_mean)

#BRMPS + Accessible:
h2_data_brmpacc <- h2_data %>%
  filter(supplement_consumed == "BRMPS+Accessible")
length(h2_data_brmpacc$participant_id) #n = 15. Small sample size.  
shapiro.test(h2_data_brmpacc$h2_mean) #Relatively normal. p = 0.05314. 
hist(h2_data_brmpacc$h2_mean)
#Normality technically passed, but it should be noted that the sample size is small. 

#HiMaize:
h2_data_himaize <- h2_data %>%
  filter(supplement_consumed == "HiMaize")
length(h2_data_himaize$participant_id) #n = 37. Moderate sample size.  
shapiro.test(h2_data_himaize$h2_mean) #Non normal. p = 9.956e-06. 
hist(h2_data_himaize$h2_mean)

#HiMaize + BRMPS:
h2_data_hibrmps <- h2_data %>%
  filter(supplement_consumed == "HiMaize+BRMPS")
length(h2_data_hibrmps$participant_id) #n = 21. Small sample size.  
shapiro.test(h2_data_hibrmps$h2_mean) #Non normal. p = 0.001864. 
hist(h2_data_hibrmps$h2_mean)

#Inulin:
h2_data_inulin <- h2_data %>%
  filter(supplement_consumed == "Inulin")
length(h2_data_inulin$participant_id) #n = 47. Moderate sample size.  
shapiro.test(h2_data_inulin$h2_mean) #Non normal. p = 0.0002929. 
hist(h2_data_inulin$h2_mean)

#LOODAT:
h2_data_loodat <- h2_data %>%
  filter(supplement_consumed == "LOODAT")
length(h2_data_loodat$participant_id) #n = 14. Small sample size.  
shapiro.test(h2_data_loodat$h2_mean) #Non normal. p = 7.089e-059. 
hist(h2_data_loodat$h2_mean)

#Psyllium:
h2_data_psyllium <- h2_data %>%
  filter(supplement_consumed == "Psyllium")
length(h2_data_psyllium$participant_id) #n = 15. Small sample size.  
shapiro.test(h2_data_psyllium$h2_mean) #Normal. p = 0.4093. 
hist(h2_data_psyllium$h2_mean)

#Psyllium+BRMPS:
h2_data_psybrmps <- h2_data %>%
  filter(supplement_consumed == "Psyllium+BRMPS")
length(h2_data_psybrmps$participant_id) #n = 14. Small sample size.  
shapiro.test(h2_data_psybrmps$h2_mean) #Normal. p = 0.472. 
hist(h2_data_psybrmps$h2_mean)

#transition_HiMaize:
h2_data_thimaize <- h2_data %>%
  filter(supplement_consumed == "transition_HiMaize")
length(h2_data_thimaize$participant_id) #n = 25. Moderate sample size.  
shapiro.test(h2_data_thimaize$h2_mean) #Normal. p = 7.102e-07. 
hist(h2_data_thimaize$h2_mean)

#Sample Size Check:
length(h2_data_acc$participant_id) + length(h2_data_ara$participant_id) + length(h2_data_ban$participant_id) + length(h2_data_brmps$participant_id) + length(h2_data_brmpacc$participant_id) + length(h2_data_himaize$participant_id) + length(h2_data_hibrmps$participant_id) + length(h2_data_inulin$participant_id) + length(h2_data_loodat$participant_id) + length(h2_data_psyllium$participant_id) + length(h2_data_psybrmps$participant_id) + length(h2_data_thimaize$participant_id)
#Result is 415. Sample sizes values computed correctly. 

#Variance Check: 
h2_data
bartlett.test(h2_data$h2_mean, h2_data$supplement_consumed) #p-value = 6.83e-08
#Variances not equal. 
```

* Which statistical test(s) would you use to answer the question? Conduct the appropriate statistical test(s):
```{r}
#Kruskal-Wallace Test should be used to answer question: "do hydrogen breath levels differ between participants consuming fiber supplements?" While some groups express normality, it should be noted that the ones that do have a small sample size, and half of the groups do not express any sort of normality. Furthermore, sample sizes differ greatly among and between groups, so an ANOVA ould not be used. For this nonparametric data, the Kruskal-Wallace test should be applied. 

kruskal.test(supplement_consumed ~ h2_mean, h2_data)
```

* What is the answer to the question? Write your conclusions:
```{r}
# conclusion 
#The Krusal-Wallis test has returned a p-value of 0.3001 With this knowledge, it can be said that based upon the specified data frame from the breath_wkly dataset, which has the same number of observations as the abridged h2_data data frame, there are no significant differences in Hydrogen breath levels between participants consuming different fiber supplements. 
```
...
m1 <- tax_scfa %>%
  filter(category == "butyrogen_focused", #subset dataframe to butyrate producers
         study_week.x == "week3", #subset for week3
...
```{r}
scfa_ratio <- tax_scfa %>%
  mutate(acetate_mean/totalscfa_mean) %>% 
  mutate(butyrate_mean/totalscfa_mean) %>% 
  mutate(propionate_mean/totalscfa_mean) %>%
  mutate(acetate_mean/totalscfa_mean + butyrate_mean/totalscfa_mean +    propionate_mean/totalscfa_mean)
scfa_ratio
```
...
Fr <- scfa_ratio %>%
  ...
  group_by(participant_id, study_week.y) %>%
  mutate(bacteroidetes_abund = sum(relative_abundance)) %>%
...
tax_ratio <- merge(Fr, Br) %>%
  mutate(f_b_ratio = (firmicutes_abund / bacteroidetes_abund)) 
  
tax_ratio_final <- merge(tax_ratio, scfa_ratio) 
tax_ratio_final
```
```
...
ggsave(path = "figures", filename = "tax_ratio_final.png")

# correlation test 
m63a <- tax_ratio_final %>%
  lm(firmicutes_abund ~ butyrate_mean, data = .)
...
# plot
tax_ratio_final %>%
  ggplot(aes(x = butyrate_mean/totalscfa_mean,
              y = f_b_ratio)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("Butyrate:Total Ratio") + 
  ylab("Firmicutes:Bacteroidetes Ratio") 
ggsave(path = "figures", filename = "butyrate:firmicutes.png")

# correlation test 
m63b <- tax_ratio_final %>%
  lm(f_b_ratio ~ butyrate_mean/totalscfa_mean, data = .)
...
tax_ratio_final %>%
  ...
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("Propionate_Conc(mmol/kg") + 
  ylab("Bacteroidetes Abundance") 
ggsave(path = "figures", filename = "propionate:bacteroidetes.png")

# correlation test 
m63b <- tax_ratio_final %>%
  lm(bacteroidetes_abund ~ propionate_mean, data = .)
summary(m63b) #view results 
...
# plot
tax_ratio_final %>%
  ggplot(aes(x = propionate_mean/totalscfa_mean,
              y = f_b_ratio)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("Propionate Ratio") + 
  ylab("Bacteroidetes:Firmicutes Ratio")
ggsave(path = "figures", filename = "propratio_bacfirmratio.png")

# correlation test 
m63b <- tax_ratio_final %>%
  lm(f_b_ratio ~ propionate_mean/totalscfa_mean, data = .)
summary(m63b) #view results 
```
...
pH_BSS_wkly <- full_join(pH_wkly, BSS_wkly)
pH_BSS_wkly <- pH_BSS_wkly %>%
  rename_all(tolower)

pH_BSS_wkly

# join 
tax_scfa_pH <- full_join(tax_ratio_final, pH_BSS_wkly) %>%
  group_by() %>%
  na.omit(semester)
```
...
tax_ratio_final %>%
  ...
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("totalscfa_mean_mmol/kg") + 
  ylab("acetate_mean_conc_mmol/kg")
ggsave(path = "figures", filename = "scfa:acetate.png")
...
tax_ratio_final %>%
  ggplot(aes(x = totalscfa_mean,
              y = acetate_mean/totalscfa_mean)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("totalscfa_mmol/kg") + 
  ylab("acetate_abundance")
ggsave(path = "figures", filename = "TOTALSCFAACETATE.png")

# correlation test 
m63b <- tax_ratio_final %>%
  lm(acetate_mean/totalscfa_mean ~ totalscfa_mean, data = .)
summary(m63b) #view results 

#No. The correlation is far weaker. 
```
...
tax_scfa_pH %>%
  ...
  xlab("totalscfa_mmol/kg") + 
  ylab("pH Value")
ggsave(path = "figures", filename = "TOTALSCFAPH.png")

# correlation test 
m63b <- tax_scfa_pH %>%
  lm(ph_mean ~ totalscfa_mean, data = .)
summary(m63b) #view results 

#Not much of a correlation, at least visually. 
...
tax_scfa_pH %>%
  ...
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlab("butyrate_mmol/kg") + 
  ylab("pH Value")
ggsave(path = "figures", filename = "BUTYRATEPH.png")

# correlation test 
m63b <- tax_scfa_pH %>%
  lm(ph_mean ~ butyrate_mean, data = .)
summary(m63b) #view results 
