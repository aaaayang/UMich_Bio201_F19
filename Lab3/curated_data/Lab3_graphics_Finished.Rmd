...
> Without facets, the graphs were not as well divided, and week 1 and week 3 looked more as separate graphs as opposed to two different facets/aspects of the same graph. This was partially because the y-axis label repeated itself on the manually done one. Furthermore, the generated graph was expanded a bit through use of geom_blank(). In other words, the above generated graph is more comphrehensive, more differntiate through is use of colors, and is more space-efficient due to further representation of the points. 
...
```{r}
plot3_6 <- scfa_qc %>%
  filter(supplement_consumed == "accessible" | supplement_consumed == "arabino" | supplement_consumed == "brmps" | supplement_consumed == "brmps+accessible" | supplement_consumed == "himaize" | supplement_consumed == "himaize+brmps" | supplement_consumed == "inulin" | supplement_consumed == "loodat" | supplement_consumed == "psyllium" | supplement_consumed == "psyllium+brmps" | supplement_consumed == "transition_himaize") %>%
  filter(study_week == "week3") %>%
  ggplot(aes(x = participant_id,
...
labs(x = "Individuals",
     y = "Mean Butyrate Presence (mmol/kg)") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")
...
```{r}
# option to save with ggplot functions 
ggplot2::ggsave(filename = "Lab3_graphics.rmd",
                path = "figures/",
                device = "tiff",
                plot = plot3_6,
                dpi = "retina")
