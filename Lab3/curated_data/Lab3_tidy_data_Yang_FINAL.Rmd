...
scfa_indv_qc <- scfa_indv %>%
  ...
  mutate(ht_m = round((ht_in/35.27), digits = 0)) %>%
  ...
  mutate(butyrate_mmol_kg = round((butyrate_mm/sample_wt_kg), digits = 2)) %>%
  mutate(propionate_mmol_kg = round((propionate_mm/sample_wt_kg), digits = 2)) %>%    
  mutate(scfa_total = acetate_mmol_kg + butyrate_mmol_kg  + propionate_mmol_kg) %>%
    select(-butyrate_mm, -propionate_mm, -acetate_mm)
```
