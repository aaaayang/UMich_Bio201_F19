...
scfa_indv_qc <- scfa_indv %>%
  ...
  mutate(Wt_lbs/2.205, round(Wt_lbs, digits = 0)) %>%
  mutate(Ht_in/35.274, round(Ht_in, digits = 0)) %>%
  filter(Wt_lbs > 0.1 & Wt_lbs <1.0) %>%
  filter(Wt_lbs = kg) %>%
  filter(notes != "frozen < 24hrs")
  mutate(Ace/tube_wt, round(Ace/tube_wt, digits = 2)) %>%
    mutate (But/tube_wt, round(But/tube_wt, digits = 2)) %>%
    mutate (Pro/tube_wt, Pro/tube_wt, digits = 2) %>%
  mutate(scfa_total = Ace/tube_wt + But/tube_wt + Pro/tube_wt) %>%
    select (-But, - Pro, - Ace)
```
...
scfa_wkly_qc <- read_excel() %>%
  scfa_wkly_qc <- rename_all(tolower) %>%
  filter("Week1" | "Week3") %>%
    filter(quantity_compliant = yes) %>%
  mean(acetate_mean + butyrate_mean + propionate_mean) %>%
  median(acetate_median + butyrate_median + propionate_median)
```
